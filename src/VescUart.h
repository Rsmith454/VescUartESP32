...
class VescUart
{
	...
	public:
		 ...
		 * @return     True if successfull otherwise false
		 */
		bool getVescValues(uint8_t data[1]);
		...
		void setAppConf(app_configuration *appconf);

		void setMcConf(mc_configuration *mc_conf);
	...
	private: 
		...
		bool unpackPayload16(uint8_t * message, int lenMes, uint8_t * payload);
		...
		bool confgeneratorDeserializeAppconf(const uint8_t *buffer, app_configuration *conf);

		int32_t confgeneratorSerializeAppconf(uint8_t *buffer, const app_configuration *conf);

		bool confgeneratorDeserializeMcconf(const uint8_t *buffer, mc_configuration *conf);

		int32_t confgeneratorSerializeMcconf(uint8_t *buffer, const mc_configuration *conf);
